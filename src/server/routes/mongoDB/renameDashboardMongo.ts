...
router.post(
    "/api/renameDashboardMongo",
    //@ts-ignore
    async (req: deleteRequest, res: any, next) => {
        try {
            ...
            if (req.session.login === true) {
                try {
                    ...
                    res.status(200).json({
                        message: `Update dashboard name complete Complete`,
                    });
                } catch (err) {
                    console.log(err);
                    res.status(500).json({
                        message: `Problem updating dashboard name in mongo.`,
                    });
                }
            }
        } catch (error) {
