...
router.get("/api/deleteFinnDashData", async (req, res) => {
    try {
        ...
        if (req.session.login === true) {
            ...
            try {
                ...
                const widgetID =
                    typeof req.query["widgetID"] === "number"
                        ? req.query["widgetID"].toString()
                        : req.query["widgetID"];
                ...
                console.log(err);
                res.status(500).json({
                    message: `Problem finding user dataset.`,
                });
            }
        }
    } catch (error) {
