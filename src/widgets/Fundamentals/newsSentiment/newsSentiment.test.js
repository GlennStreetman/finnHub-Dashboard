...
//mock routes
import {mockFinnHubData, getDashboard_success, getCheckLogin_success, postFindMongoData_success_noData, postUpdateGQLFilters} from './newsSentiment.mock'
...
it('Test Single Widget dashboard : news sentiment ', async (done) => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('News Sentiments')).toBeInTheDocument() //copy submenu text
    })
    expect(getByTestId('News Sentiments')).toBeInTheDocument() //copy submenu text
    fireEvent.click(screen.getByTestId('News Sentiments')) //copy submenu text
    //check widget is mounted
    await waitFor(() => {
        expect(screen.getByTestId('newsSentimentBody')).toBeInTheDocument() //testID for widgets outer most div
        //additional tests for widget body render.
    })
    // check that edit pane for widget renders and all buttons work
    expect(screen.getByTestId('editPaneButton-FundamentalsNewsSentiment')).toBeInTheDocument() //update with widgetID
    fireEvent.click(getByTestId('editPaneButton-FundamentalsNewsSentiment')) //update with widgetID
    await waitFor(() => {
    ...
    //unmount widget
    expect(screen.getByTestId('removeWidget-FundamentalsNewsSentiment')).toBeInTheDocument()  //update with widgetID
    fireEvent.click(screen.getByTestId('removeWidget-FundamentalsNewsSentiment')) //update with widgetID
    await waitFor(() => {
        expect(screen.queryByTestId('newsSentimentBody')).toBe(null) //update widget body tag.
    })
    done()
})
