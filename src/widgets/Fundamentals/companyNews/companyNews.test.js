...
//mock routes
import {mockFinnHubData, getDashboard_success, getCheckLogin_success, postFindMongoData_success_noData, postUpdateGQLFilters} from './companyNews.mock'
...
it('Test Single Widget dashboard : Company News ', async (done) => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('Company News')).toBeInTheDocument() //copy submenu text
    })
    expect(getByTestId('Company News')).toBeInTheDocument() //copy submenu text
    fireEvent.click(screen.getByTestId('Company News')) //copy submenu text
    //check widget is mounted
    await waitFor(() => {
        expect(screen.getByTestId('companyNewsBody')).toBeInTheDocument() //testID for widgets outer most div
        expect(screen.getByTestId('newBodyTable')).toBeInTheDocument()
    })
    // check that edit pane for widget renders and all buttons work
    expect(screen.getByTestId('editPaneButton-FundamentalsCompanyNews')).toBeInTheDocument() //update with widgetID
    fireEvent.click(getByTestId('editPaneButton-FundamentalsCompanyNews')) //update with widgetID
    await waitFor(() => {
    ...
    //unmount widget
    expect(screen.getByTestId('removeWidget-FundamentalsCompanyNews')).toBeInTheDocument()  //update with widgetID
    fireEvent.click(screen.getByTestId('removeWidget-FundamentalsCompanyNews')) //update with widgetID
    await waitFor(() => {
        expect(screen.queryByTestId('companyNewsBody')).toBe(null) //update widget body tag.
    })
    done()
})
