...
//mock routes
import {mockFinnHubData, getDashboard_success, getCheckLogin_success, postFindMongoData_success_noData, postUpdateGQLFilters} from './peers.mock'
...
it('Test Single Widget dashboard : peers ', async (done) => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('Peers')).toBeInTheDocument() //copy submenu text
    })
    expect(getByTestId('Peers')).toBeInTheDocument() //copy submenu text
    fireEvent.click(screen.getByTestId('Peers')) //copy submenu text
    //check widget is mounted
    await waitFor(() => {
        expect(screen.getByTestId('peersBody')).toBeInTheDocument() //testID for widgets outer most div
        //additional tests for widget body render.
    })
    // check that edit pane for widget renders and all buttons work
    expect(screen.getByTestId('editPaneButton-FundamentalsPeers')).toBeInTheDocument() //update with widgetID
    fireEvent.click(getByTestId('editPaneButton-FundamentalsPeers')) //update with widgetID
    await waitFor(() => {
    ...
    //unmount widget
    expect(screen.getByTestId('removeWidget-FundamentalsPeers')).toBeInTheDocument()  //update with widgetID
    fireEvent.click(screen.getByTestId('removeWidget-FundamentalsPeers')) //update with widgetID
    await waitFor(() => {
        expect(screen.queryByTestId('peersBody')).toBe(null) //update widget body tag.
    })
    done()
})
