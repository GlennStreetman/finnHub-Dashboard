...
const body = 'basicFinancialsBody'
...
beforeEach( async ()=>{
    ...
    await addWidget('estimatesDropdown', 'Fundamentals', 'Basic Financials') //mount widget to be tested.
})
...
it(`Test ${widgetType} Widget: Select metrics. Check data renders.`, async (done) => {
    ...
    await testBodyRender([ //does a chart render?
        ['getByText','Show:'],
...
it(`Test ${widgetType} Widget: Add security from widget config screen works.`, async (done) => { 
    ...
    await addSecurity(widgetType, [['TSLA', 'US-TSLA: TESLA INC'], ['AAPL', 'US-AAPL: APPLE INC']]) //add security to widget with search bar. Tests change took effect on each iteration.
    await waitFor(() => {
