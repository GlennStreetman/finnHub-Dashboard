...
//mock routes
import {mockFinnHubData, getDashboard_success, getCheckLogin_success, postFindMongoData_success_noData, postUpdateGQLFilters} from './priceSplits.mock'
...
it('Test Single Widget dashboard : priceSplits ', async (done) => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('Splits')).toBeInTheDocument() //copy submenu text
    })
    expect(getByTestId('Splits')).toBeInTheDocument() //copy submenu text
    fireEvent.click(screen.getByTestId('Splits')) //copy submenu text
    //check widget is mounted
    await waitFor(() => {
        expect(screen.getByTestId('splitsBody')).toBeInTheDocument() //testID for widgets outer most div
        //additional tests for widget body render.
    })
    // check that edit pane for widget renders and all buttons work
    expect(screen.getByTestId('editPaneButton-PriceSplits')).toBeInTheDocument() //update with widgetID
    fireEvent.click(getByTestId('editPaneButton-PriceSplits')) //update with widgetID
    await waitFor(() => {
    ...
    //unmount widget
    expect(screen.getByTestId('removeWidget-PriceSplits')).toBeInTheDocument()  //update with widgetID
    fireEvent.click(screen.getByTestId('removeWidget-PriceSplits')) //update with widgetID
    await waitFor(() => {
        expect(screen.queryByTestId('splitsBody')).toBe(null) //update widget body tag.
    })
    done()
})
