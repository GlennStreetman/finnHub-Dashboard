...
//mock routes
import {mockFinnHubData, getDashboard_success, getCheckLogin_success, postFindMongoData_success_noData, postUpdateGQLFilters} from './candle.mock'
...
it('Test Single Widget dashboard : EarningsCalendar ', async (done) => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('Candles')).toBeInTheDocument() //copy submenu text
    })
    expect(getByTestId('Candles')).toBeInTheDocument() //copy submenu text
    fireEvent.click(screen.getByTestId('Candles')) //copy submenu text
    //check widget is mounted
    await waitFor(() => {
        expect(screen.getByTestId('candleBody')).toBeInTheDocument() //testID for widgets outer most div
        //additional tests for widget body render.
    })
    // check that edit pane for widget renders and all buttons work
    expect(screen.getByTestId('editPaneButton-PriceCandles')).toBeInTheDocument() //update with widgetID
    fireEvent.click(getByTestId('editPaneButton-PriceCandles')) //update with widgetID
    await waitFor(() => {
    ...
    //unmount widget
    expect(screen.getByTestId('removeWidget-PriceCandles')).toBeInTheDocument()  //update with widgetID
    fireEvent.click(screen.getByTestId('removeWidget-PriceCandles')) //update with widgetID
    await waitFor(() => {
        expect(screen.queryByTestId('candleBody')).toBe(null) //update widget body tag.
    })
    done()
})
