...
//mock routes
import { mockFinnHubData, getDashboard_success, mockExchangeData } from "./candle.mock"; //if premium route add checkLogin override to .mock.ts file.
...
const widgetType = "PriceCandles";
const body = "container-candleBody";
...
beforeEach(async () => {
    ...
    await addWidget("Stock Price", "Price Candle", body); //mount widget to be tested.
});
...
it(`Test ${widgetType} Widget: Change focus renders body change. `, async () => {
    await testBodyRender([
        ...
        ["getByTestId", body],
        ["getByText", "US-WMT: Weekly Candle Data"],
    ]);
    console.log("starting security found");
    ...
    await testBodyRender([
        ...
        ["getByTestId", body],
        ["getByText", "US-COST: Weekly Candle Data"],
    ]);
