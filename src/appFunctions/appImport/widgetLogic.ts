...
export const UpdateWidgetFilters = async function (
    ...
    try {
        ...
        fetch(`/api/deleteFinnDashData?widgetID=${widgetID}`);
        dispatch(
            rRebuildTargetWidgetModel({
                //rebuild data model (removes stale dates)
                apiKey: apiKey,
                dashBoardData: dashBoardData,
        ...
        console.log("Problem updating widget filters.");
        return false;
    }
};
...
export const updateWidgetConfig = async function (
    ...
    if (enableDrag !== true) {
        const updatedDashboardData: dashBoardData = produce(
            dashBoardData,
            (draftState: dashBoardData) => {
                const oldConfig =
                    draftState[currentDashboard].widgetlist[widgetID].config;
                draftState[currentDashboard].widgetlist[widgetID].config = {
                    ...oldConfig,
                    ...updateObj,
                };
        ...
        dispatch(tSaveDashboard({ dashboardName: currentDashboard }));
        const updatedWidgetFilters =
            updatedDashboardData[currentDashboard].widgetlist[widgetID].config;
        ...
        fetch("/api/updateGQLConfig", options);
    }
};
...
export const toggleWidgetBody = function (
    ...
    if (stateRef === "stockWidget") {
        const updatedWidget: dashBoardData = produce(
            dashBoardData,
            (draftState: dashBoardData) => {
                draftState[currentDashboard].widgetlist[widgetID].showBody =
                    !draftState[currentDashboard].widgetlist[widgetID].showBody;
            }
        );
        return updatedWidget;
    } else {
        const updatedWidget: menuList = produce(
            menuList,
            (draftState: menuList) => {
                draftState[widgetID].showBody =
                    draftState[widgetID].showBody !== undefined
                        ? !draftState[widgetID].showBody
                        : false;
            }
        );
