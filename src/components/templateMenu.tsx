...
export default function TemplateMenu(p: "pass", ref: any) {
    ...
    function deleteFile(e) {
        fetch(`/api/deleteTemplate?template=${e}`).then(() => {
    ...
    function runTemplate(e, apiKey, templateName, checked) {
        fetch(
            `/api/runTemplate?key=${apiKey}&template=${templateName}&multi=${isChecked(
                checked
            )}`
        )
    ...
    function templateTable(tempList: string[][]) {
        ...
        return tempList.map((el) => (
            <TableRow key={el[0] + "tr"}>
                <TableCell key={el[0] + "td1"}>
                    {el[0].replace(".xlsx", "")}
                </TableCell>
                <TableCell key={el[0] + "td2"}>
                    <a
                        href="#template"
                        onClick={(e) => {
                            runTemplate(e, keySwitch, el[1], el[0]);
                        }}
                    >{`/runTemplate?key=${keySwitch}&template=${
                        el[1]
                    }&multi=${isChecked(el[0])}`}</a>
                </TableCell>
                ...
                <TableCell key={el[0] + "td4"}>
                    <button onClick={() => deleteFile(el[0])}>
