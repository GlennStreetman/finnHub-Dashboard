...
function ManageWidgets() {
    ...
    const dashboardSelections = dashboardList.map((el) => (
        <MenuItem value={el} key={el + "dbSelection"}>
            {el}
        </MenuItem>
    ));

    const columnSelections = [...Array(7).keys()].map((el) => (
        <MenuItem value={el} key={el + "columnSelections"}>
            {el}
        </MenuItem>
    ));
    ...
    const widgetHeadings = Object.keys(widgetLookup).map((el) => (
        <MenuItem data-testid={`${el}-selection`} value={el} key={el + "headerSelect"}>
            {el}
        </MenuItem>
    ));
    ...
    const addWidgets = () => {
        if (dashboardData[currentDashboard]) {
            ...
            return currentSelection.map((el) => {
                const thisCount = Object.values(dashboardData[currentDashboard].widgetlist).reduce((r, w) => {
                ...
                return (
                    <Card raised={true} key={"card" + el[1]}>
                        <CardContent>
                            <Typography>
                                <b>{el[1]}:</b> {widgetDescriptions[el[0]]}
                            </Typography>
                            {/* @ts-ignore */}
                            <div style={cardContainer}>
                                ...
                                <div style={cardStyle}>
                                    <Button
                                        ...
                                        variant="contained"
                                        onClick={() => addWIdget(el, currentDashboard)}
                                    >
                                        Add
                                    </Button>
                                </div>
    ...
    const manageWidgetPlacement = () => {
        if (dashboardData[currentDashboard]) {
            ...
            return widgetList.map((el) => {
                ...
                const columnWidgets = widgetList.reduce((r, w) => {
                    //NUMBER OF WIDGETS IN COLUMN
                ...
                const orderSelect = columnWidgets.map((el) => (
                    <MenuItem value={el.columnOrder} key={el.widgetID + "columnSelections"}>
                        {el.columnOrder + 1}
                    </MenuItem>
                ));

                return (
                    <Card raised={true} key={"card" + el.widgetHeader}>
                        <CardContent>
                            <Typography>
                                <b>{el.widgetHeader}:</b> {widgetDescriptions[el.widgetType]}
                            </Typography>
                            {/* @ts-ignore */}
                            <div style={cardContainer}>
                                <div style={cardStyle}>
                                    <Typography>Column:</Typography>
                                    <Select value={el.column} onChange={(e) => handleColumnChange(e, currentDashboard, el.widgetID)}>
                                        {columnSelections}
                                    </Select>
                                </div>
                                <div style={cardStyle}>
                                    <Typography>Order:</Typography>
                                    <Select value={el.columnOrder} onChange={(e) => handleOrderChange(e, currentDashboard, el.widgetID, el.column)}>
                                        {orderSelect}
                                    </Select>
                                </div>
                            </div>
        ...
        <div>
            <div className={classes.root}>
                ...
                <div style={background}>
                    <Tabs value={toggle} onChange={handleChange} indicatorColor="primary" textColor="primary">
            ...
            <div style={flexInline}>
                <FormControl variant="outlined">
                    ...
                    <Select labelId="Dashboard-Label" value={dashboard} onChange={handDashboardChange} data-testid="manageWidgets-SelectDashboard">
                        {dashboardSelections}
                    </Select>
                </FormControl>
                <FormControl variant="outlined">
                    ...
                    <Select labelId="API-Heading-Label" value={widgetHeading} onChange={handleHeadingChange} data-testid="manageWidgets-SelectWidgetGroup">
                        {widgetHeadings}
                    </Select>
                </FormControl>
            </div>
            ...
            <div style={flexInline}>
                {/* @ts-ignore */}
                <div style={cardColumn}>{toggle === 0 ? addWidgets() : manageWidgetPlacement()}</div>
            </div>
