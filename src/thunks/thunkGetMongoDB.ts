...
export const tGetMongoDB = createAsyncThunk(
    //{endPoint, [securityList]}
    "tgetMongoDb",
    async (reqObj: tgetMongoDBReq | false = false) => {
        try {
            let fetchString = "/api/getFinnDashDataMongo";
            if (reqObj) {
                //build request string with filters.
                fetchString = fetchString + "?";
                if (reqObj.dashboard)
                    fetchString = `${fetchString}dashboardID=${reqObj.dashboard}`;
                if (reqObj.widget)
                    fetchString = `${fetchString}&widget=${reqObj.widget}`;
            }
